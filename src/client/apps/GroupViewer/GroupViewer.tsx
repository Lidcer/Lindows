...
import { GroupViewerCanvas, CanvasInteraction } from './GroupViewerCanvas';
...
export class GroupViewer extends BaseWindow<GroupViewerState> {
  ...
  connect = async () => {
    ...
    if (this.variables.connectUser && this.variables.connectPassword) {
      ...
      try {
        const exist = await this.network.emitPromise<string, [string, string]>('group-viewer-connect', this.variables.connectUser, this.variables.connectPassword);
        if(!exist) {
  ...
  renderInside() {
    ...
    if(this.variables.connectionEstablished){
      return <GroupViewerCanvas canvasInteraction={((canvasInteraction) => this.canvasInteraction = canvasInteraction)}/>
    ...
    const e = (<div style={{border: '1px solid blue'}}>
    <div>
      <span>User ID:</span>
